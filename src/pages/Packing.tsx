...
import {
  ...
  ListChecks, Truck, ArrowDownToLine, CheckCircle, Clock, Gem,
} from 'lucide-react';
...
export default function PackingPage() {
  ...
  const [viewTab, setViewTab] = useState<'3d' | 'load' | 'unload'>('3d');
  const [loadChecked, setLoadChecked] = useState<Set<string>>(new Set());
  const [unloadChecked, setUnloadChecked] = useState<Set<string>>(new Set());
  ...
  const toggleLoadCheck = async (boxId: string) => {
    const newSet = new Set(loadChecked);
    if (newSet.has(boxId)) {
      newSet.delete(boxId);
    } else {
      newSet.add(boxId);
      await confirmBoxLoaded(boxId);
    }
    setLoadChecked(newSet);
  };

  const toggleUnloadCheck = async (boxId: string) => {
    const newSet = new Set(unloadChecked);
    if (newSet.has(boxId)) {
      newSet.delete(boxId);
    } else {
      newSet.add(boxId);
      await confirmBoxUnloaded(boxId);
    }
    setUnloadChecked(newSet);
  };

  useEffect(() => {
    const loadedIds = new Set(boxes.filter(b => b.status >= 3).map(b => b.id));
    setLoadChecked(loadedIds);
    const unloadedIds = new Set(boxes.filter(b => b.status >= 4).map(b => b.id));
    setUnloadChecked(unloadedIds);
  }, [boxes]);
  ...
  const packedBoxCount = packingResult.packedBoxes.length;
  const loadProgress = packedBoxCount > 0 ? Math.round((loadChecked.size / packedBoxCount) * 100) : 0;
  const unloadProgress = packedBoxCount > 0 ? Math.round((unloadChecked.size / packedBoxCount) * 100) : 0;
  const loadedBoxes = boxes.filter(b => b.status >= 3);
  const unloadedBoxes = boxes.filter(b => b.status >= 4);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card !p-1.5 bg-slate-100 inline-flex rounded-2xl">
        {[
          { k: '3d', l: '3D可视化', i: Layers, badge: null },
          { k: 'load', l: '装车执行清单', i: Truck, badge: `${loadChecked.size}/${packedBoxCount}` },
          { k: 'unload', l: '卸货核对', i: ArrowDownToLine, badge: `${unloadChecked.size}/${packedBoxCount}` },
        ].map(t => {
          const TIcon = t.i;
      ...
      {viewTab === 'load' && (
        <div className="card !p-0 overflow-hidden">
          <div className="p-5 border-b border-slate-100 bg-gradient-to-r from-primary-50/60 to-accent-50/40">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <h3 className="font-display font-bold text-lg text-slate-900 flex items-center gap-2 mb-1">
                  <Truck className="w-5 h-5 text-primary-600" /> 装车执行清单
                </h3>
                <p className="text-sm text-slate-500">基于装箱算法的最优装载顺序，勾选确认实际已装车</p>
              </div>
              <div className="text-right">
                <div className="font-display font-bold text-3xl text-primary-700">{loadProgress}%</div>
                <div className="text-xs text-slate-500">装车进度 · {loadChecked.size}/{packedBoxCount} 箱</div>
              </div>
            </div>
          ...
          <div className="divide-y divide-slate-50">
            {packingResult.loadSequence.map((boxId, idx) => {
              ...
              const room = rooms.find(r => r.id === box.room_id);
              const color = roomColorMap.get(box.room_id) || '#64748b';
              const isChecked = loadChecked.has(boxId);
              const boxItems = getItemsByBox(boxId);
              return (
                <div
                  key={boxId}
                  className={`p-4 flex items-start gap-4 transition-all ${isChecked ? 'bg-emerald-50/40' : ''}`}
                >
                  <button
                    onClick={() => toggleLoadCheck(boxId)}
                  ...
                  <div className="w-10 h-10 rounded-xl bg-primary-100 text-primary-700 font-bold flex items-center justify-center shrink-0">
                    #{idx + 1}
                  </div>
                  <div className="w-1 flex-1 rounded-full shrink-0" style={{ backgroundColor: color, minWidth: 4 }} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={`font-display font-bold text-lg ${isChecked ? 'text-emerald-700 line-through opacity-70' : 'text-slate-900'}`}>
                        {box.box_code}
                      </span>
                      {box.is_fragile && <Gem className="w-3.5 h-3.5 text-violet-500" />}
                      {box.is_overweight && <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-100 text-rose-700 font-semibold">超重</span>}
                    </div>
                    <div className="text-xs text-slate-500 mb-2">
                      {room?.name} · {box.items_count}件 · {box.weight_kg}kg · {((box.length_cm * box.width_cm * box.height_cm) / 1e6).toFixed(2)}m³
                    </div>
                    {boxItems.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {boxItems.slice(0, 5).map(it => (
                  ...
                  <div className="text-right shrink-0">
                    {isChecked && box.loaded_at && (
                      <div className="text-[11px] text-emerald-600 font-medium flex items-center gap-0.5">
                        <Clock className="w-3 h-3" />
                        {new Date(box.loaded_at).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    )}
                    <div className="text-[10px] text-slate-400 mt-1">
                      坐标 {Math.round(box.pos_x)},{Math.round(box.pos_y)},{Math.round(box.pos_z)}
                    </div>
                  </div>
                </div>
              );
      ...
      {viewTab === 'unload' && (
        <div className="card !p-0 overflow-hidden">
          <div className="p-5 border-b border-slate-100 bg-gradient-to-r from-violet-50/60 to-emerald-50/40">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <h3 className="font-display font-bold text-lg text-slate-900 flex items-center gap-2 mb-1">
                  <ArrowDownToLine className="w-5 h-5 text-violet-600" /> 卸货核对清单
                </h3>
                <p className="text-sm text-slate-500">到达目的地后，按反向装载顺序卸货并逐一核对</p>
              </div>
              <div className="text-right">
                <div className="font-display font-bold text-3xl text-violet-700">{unloadProgress}%</div>
                <div className="text-xs text-slate-500">卸货进度 · {unloadChecked.size}/{packedBoxCount} 箱</div>
              </div>
            </div>
            <div className="mt-4 h-2 bg-white rounded-full overflow-hidden">
              <div
          ...
          <div className="divide-y divide-slate-50">
            {[...packingResult.loadSequence].reverse().map((boxId, idx) => {
              const box = packingResult.packedBoxes.find(b => b.id === boxId);
              if (!box) return null;
              const room = rooms.find(r => r.id === box.room_id);
              const color = roomColorMap.get(box.room_id) || '#64748b';
              const isChecked = unloadChecked.has(boxId);
              const boxItems = getItemsByBox(boxId);
              return (
                <div
                  key={boxId}
                ...
                >
                  <button
                    onClick={() => toggleUnloadCheck(boxId)}
                    className={`mt-0.5 w-6 h-6 rounded-md border-2 flex items-center justify-center shrink-0 transition ${
                      isChecked
                        ? 'bg-emerald-500 border-emerald-500'
                  ...
                  <div className="w-10 h-10 rounded-xl bg-violet-100 text-violet-700 font-bold flex items-center justify-center shrink-0">
                    #{idx + 1}
                  </div>
                  <div className="w-1 flex-1 rounded-full shrink-0" style={{ backgroundColor: color, minWidth: 4 }} />
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500 mb-2">
                      搬入 → {room?.name} · {box.items_count}件 · {box.weight_kg}kg
                    </div>
                    {boxItems.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {boxItems.slice(0, 5).map(it => (
                  ...
                  <div className="text-right shrink-0">
                    {isChecked && box.unloaded_at && (
                      <div className="text-[11px] text-emerald-600 font-medium flex items-center gap-0.5">
                        <Clock className="w-3 h-3" />
                        {new Date(box.unloaded_at).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    )}
                  </div>
                </div>
